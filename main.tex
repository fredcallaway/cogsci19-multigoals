...
\section{Background}
Much previous work in psychology has studied the strategies people use to make planning and problem solving tractable. One class of strategies resemble algorithms for \textit{tree search}, in which possible plans are simulated forward and evaluated~\cite{}. For example, rather than consider plans that are arbitrarily long, people can engage in \textit{depth-limited search} that focuses resources on plans that can be taken within a certain time horizon~\cite{MacGregor2001,Keramati2016,Krusche2018}. Alternatively, they may rely on heuristics to guide search~\cite[chap. 5]{Anderson1990} or use local signals of progress to avoid less promising plans without explicitly considering them~\citep{Huys2012}.

Another class of strategies involves \textit{hierarchically decomposing} a problem into a set of sub-problems, and then solving those sub-problems in the right order~\cite{sacerdoti1974planning, botvinick2012hierarchical}. This approach can make decision-making easier by reducing the resources needed to solve a given sub-problem at a particular time~\citep{VanDijk2011, Maisto2015}, however, it also introduces the question of how people identify a good hierarchy in the first place. 
...
Participants completed three tutorial trials of increasing difficulty (a 3-block trial, a 4-block trial, and a 5-block trial) and then completed sixteen 6-block trials. Only the 6-block trials are analyzed below. Problems were selected to \red{maximize the difference in likelihood of model fits between simulated agents with limits on goal representation ($k < \MISSING$) and depth-limited agents ($d < \MISSING$) when fit to trials simulated by agents with limits on goal representation.}
...
%
\begin{equation}\label{likelihood-fixed}
...
\section{Discussion}
first paragraph reminds people of big problem, summarizes result.

To solve complex problems, people have to break them down into smaller problems. The importance of subgoals has been long recognized, and they play a critical role in the most successful models of human problem solving \citep{anderson2013architecture,NewellSimon1972,laird1987soar}. However, these models are limited by the assumption that only one subgoal can be active at a time

use rest of discussion to discuss limitations of analysis & directions for future work
implications, you can come back to and say by showing this, we gain this new insight, it's relevant here and here

rule: if you start with colorful example, you have to return in the final sentence.
