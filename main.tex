...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=6cm]{example-suss}
    \caption{Depicts Sussman's anomaly. In this block stacking problem, the aim is to stack the blocks in alphabetical order by satisfying two subgoals: stack B on C and stack A on B. Planning towards one subgoal is inefficient by taking actions that will need to be reversed. Only by jointly considering the subgoals can the problem be efficiently solved.}
    \label{fig:suss}
\end{figure*}
...
Models were fit for each participant, see Figure \ref{fig:fits}.
